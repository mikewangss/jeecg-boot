...
//        Map<String, Object> values = taskVo.getValues();
        ...
        if (task2 != null && task.getTaskDefinitionKey().equals(task2.getTaskDefinitionKey())) {
            ...
            if (nextFlowNode != null) {
                ...
                if (nextFlowNodeUserList.size() > 0) {
                    collect_username = nextFlowNodeUserList.stream()
                            .filter(Objects::nonNull)
                            .map(SysUser::getUsername)
                            .filter(Objects::nonNull)  // 过滤掉可能为null的用户名
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
        ...
        String activityId = execution.getActivityId();// 获取流程实例的当前执行节点ID
    ...
    public Result finishedList(Integer pageNum, Integer pageSize) {
        ...
        for (HistoricTaskInstance histTask : historicTaskInstanceList) {
            ...
            flowTask.setAssigneeId(histTask.getAssignee());
            flowTask.setCategory(pd.getCategory());
            // 流程发起人信息
            HistoricProcessInstance historicProcessInstance = historyService.createHistoricProcessInstanceQuery()
...
//        result.put("finished",true);
                ...
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        ...
                        if (sysUser != null) {
                            ...
                            flowTask.setAssigneeId("admin");
                            flowTask.setAssigneeName("机器人");
                        }
     ...
     * 流程历史流转记录
     *
     * @param procInsId 流程数据Id
     * @return
     */
    @Override
    public Result flowRecord2(String procInsId) {
        FlowMyBusiness business = flowMyBusinessService.getByProcessInstanceId(procInsId);
        Map<String, Object> map = new HashMap<String, Object>();
        if (StringUtils.isNotBlank(procInsId)) {
            List<HistoricActivityInstance> list = historyService
                    .createHistoricActivityInstanceQuery()
     ...
     * @param taskId 当前节点id
     * @param values 流程变量
     * @return 如果返回null，表示没有下一个节点，流程结束，flowNextDto.setUserList , flowNextDto.setUserTask
     */
...
//                        { //汪伟 ： 如果分配用户不为空，因为指派用户新增了岗位和角色，注意单个实例只能配置一个用户，此时会覆盖
//                            if (!assignee.contains("用户")) { //非用户使用多实例分配任务，所以不需要assignee
//                                userTask.setAssignee("");
//                            }
//                        }
//                    }
    ...
    public List<SysUser> getSysUserFromTask(UserTask userTask, Map<String, Object> values) {
