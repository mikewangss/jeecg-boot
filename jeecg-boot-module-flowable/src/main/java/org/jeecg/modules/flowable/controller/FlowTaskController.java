...
public class FlowTaskController {
    ...
    public Result flowRecord(String process_instance_id, String dataId) {
        if (StringUtils.isNullOrEmpty(dataId) && StringUtils.isNullOrEmpty(process_instance_id)) {
            return Result.error("请检查请求参数");
        }
        if (StringUtils.isNullOrEmpty(dataId)) {
            return flowTaskService.flowRecord2(process_instance_id);
        } else {
