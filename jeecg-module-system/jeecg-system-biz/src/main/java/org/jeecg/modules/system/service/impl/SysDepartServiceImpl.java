...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    @Override
    public List<SysDepartTreeModel> queryTreeByRegion(String region,String orgCode) {
        LambdaQueryWrapper<SysDepart> query = new LambdaQueryWrapper<SysDepart>();
        query.eq(SysDepart::getDelFlag, CommonConstant.DEL_FLAG_0.toString());
        query.orderByAsc(SysDepart::getDepartOrder);
        List<SysDepart> list = this.list(query);
        ...
        if (StringUtils.isNotBlank(region)) {
            this.getTreeByRegion(region, orgCode,listResult, treelist);
        } else {
            return listResult;
        }
        return treelist;
     ...
     * @return
     */
    public void getTreeByRegion(String region,String orgCode, List<SysDepartTreeModel> allResult, List<SysDepartTreeModel> newResult) {
     ...
     */
    private void findPath(ExportDepartVo departVo, List<String> path, Integer
            tenantId, List<ExportDepartVo> childrenDepart, List<String> departIdList) {
        //获取租户id和部门父id获取的部门数据
     ...
     */
    private Boolean addDepartByName(String departNameUrl, String departName, SysDepart
            sysDepart, List<String> errorMessageList, Integer tenantId, Map<String, SysDepart> departMap, int num) {
     ...
     */
    private String getDepartListByName(String departName, Integer tenantId, SysDepart sysDepart, String[]
            departNameUrls, int count, int departNum, String name, Map<String, SysDepart> departMap) {
        //递归查找下一级
        //update-begin---author:wangshuai ---date:20230721  for：一个租户部门名称可能有多个------------
        SysDepart parentDept = null;
